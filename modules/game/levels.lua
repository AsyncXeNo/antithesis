...
function TimelineSystem:update(dt)
    ...
    for _, e in ipairs(self.generalPool) do
        ...
        elseif action.type == "move and shoot" then

            if(e.Position == nil or e.Stats == nil) then
                log.error("Cannot perform action \"move and shoot\" without Position and Stats components")
                e.Timeline.curIndex = e.Timeline.curIndex + 1
            end
        
        elseif action.type == "spawn" then

            if(e.Spawner == nil) then
                log.error("Entity other than a Spawner is attempting to spawn")
                e.Timeline.curIndex = e.Timeline.curIndex + 1
            end
    ...
    for _,e in ipairs(self.moveNShootPool) do
        local action = e.Timeline.actions[e.Timeline.curIndex]

        if (action.type == "move and shoot") then
    ...
    for _,e in ipairs(self.spawnPool) do
        local action = e.Timeline.actions[e.Timeline.curIndex]
